...
contract MathLibrary {
    ...
    function divFloat(string memory _str1, string memory _str2)
        public
        pure
        returns (
            uint256 wp,
        ...
        )
    {
        ...
        require(
            dp != 0, "Cannot divide by zero"
        ); 

        wp = wp * 10**(30) * 10**(lengthOfString(dpStr2)) * 10**(lengthOfUint(dp)) / (dp * 10**(lengthOfString(dpStr1))) ;
            ...
            _str = string(
                abi.encodePacked(
                    substring(
                        _str,
                        0,
                        lengthOfString(_str) - (30 + lengthOfUint(dp))
                    ),
                    ".",
                    substring(
                        _str,
    ...
    function toPowerOf(string memory _str, uint256 _powerRoot) public pure returns(uint256 wp, uint256 dp, string memory str) {
        if (_powerRoot == 0) {
            str = "1.0";
        } else {
            str = _str;
    ...
    function powerRoot(uint256 _b, string memory _x) public pure returns(string memory str) {
        ...
        str = string(
            abi.encodePacked(
                bytes(str)[0],
                ".",
                substring(str, 1, lengthOfString(str) - 1)
            )
        );
    }
